document.addEventListener("DOMContentLoaded", () => {
  ...
  db.collection("users").doc(uid).get().then(doc => {
    ...
    postRef.set({
      id: postRef.key,
      author: uid,
      authorName: name,  // ✅ Onjezani dzina
      content,
      likes: 0,
      timestamp: Date.now()
    });
...
posts.forEach(post => {
  ...
  rtdb.ref(`posts/${post.id}/comments`).once("value", snap => {
    ...
    div.innerHTML = `
      <p><strong>${post.authorName}</strong></p>  <!-- ✅ Dzina la mwini post -->
      <p>${preview}</p>
      ${post.content.length > 100 ? `<button onclick="viewFullPost('${post.id}')">See more</button>` : ""}
      <div style="display:flex; gap:6px; margin-top:6px;">
        <button style="flex:1;" onclick="likePost('${post.id}')">❤️ (${post.likes || 0})</button>
  ...
  rtdb.ref(`posts/${postId}/comments/${commentId}/replies`).push({
    ...
    input.value = "";
    loadReplies(postId, commentId); // refresh replies
  });
 };
  ...
  // 🔁 Bwezerani ma comments omwe analipo kale
  loadComments(postId);
 };
